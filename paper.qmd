...
fert_2023 <- fert_per_prov %>%
  filter(
    REF_DATE == 2023,
    grepl("total fertility rate", Characteristics, ignore.case = TRUE)
  )

# ==== Step 3: Select relevant columns (Province, Year, and Value) ====
fert_2023_summary <- fert_2023 %>%
  select(GEO, REF_DATE, Characteristics, VALUE)



# ==== Step 3: Clean province names ====
fert_2023_summary <- fert_2023_summary %>%
  mutate(
    GEO_clean = GEO %>%
      str_replace(", place of residence of mother", "") %>%
      str_trim()
  )
...
# ==== Step 5: Manual name reconciliation ====
# Ensure names match exactly between GEO_clean and canada$name
name_map <- c(
...
fert_2023_summary <- fert_2023_summary %>%
  mutate(GEO_match = recode(GEO_clean, !!!name_map))

# ==== Step 6: Join with map ====
canada_map <- canada %>%
  left_join(fert_2023_summary, by = c("name" = "GEO_match"))

# ==== Step 7: Verify missing provinces ====
print(setdiff(canada$name, canada_map$name[!is.na(canada_map$VALUE)]))
...
ggplot(fertility_postWWII, aes(x = Year)) +
  ...
  annotate("text", x = 1969, y = 3.0, label = "1969: Decriminalization of\ncontraception & abortion",
           color = "black", size = 3.5, hjust = 0) +
  annotate("text", x = 2015, y = 1.8, label = "2020: COVID\n-19 pandemic",
           color = "black", size = 3.5, hjust = 0) +
