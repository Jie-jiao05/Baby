...
# --- Step 1: 读取并清洗数据 ---
death <- read_csv(here("Canada_Death.csv"), show_col_types = FALSE)
...
# --- Step 2: Filter for 2000–2022, age 0, Canada, and selected causes ---
death_dot <- death %>% filter(country == "Canada",
         year >= 2000, year <= 2022,
         age_group == "0",
         cause %in% c("I050", "I051", "I058")) %>%
  group_by(year, cause) %>%
...
province_centroids <- province_centroids %>%
  mutate(
    X = case_when(
      abbrev == "NU" ~ X - 5,       # shift left
      abbrev == "NT" ~ X,           # keep x position
      TRUE ~ X
    ),
    Y = case_when(
      abbrev == "NU" ~ Y - 6,     # shift down
      abbrev == "NT" ~ Y - 2.5,     # shift slightly down
      TRUE ~ Y
    )
  )
...
ggplot(death_dot, aes(x = year, y = total_deaths, color = cause)) +
  ...
  labs(
    title = "Infant Mortality in Canada (2000–2022)",
    subtitle = "Age group: 0 years; Three selected causes with individual gray trend lines",
    x = "Year",
    y = "Total Deaths"
  ) +
